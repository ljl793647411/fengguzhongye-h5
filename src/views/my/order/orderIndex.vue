        ...
        <div class="itemList" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
          <a v-for="(item,index) in orderlist" :key="index">
            <div class="head flexBetween">
              ...
              <span v-else-if="item.f_logistics==6">已评论</span>
            </div>
            <div
            ...
            <div class="footer">
              ...
              <div class="closeOrder" v-if="item.f_logistics==1 || item.f_logistics==2&&agentype==1">
                <span class="btn" @click="cancelorder(item.id)">关闭订单</span>
                <span
                v-if="item.f_logistics==1&&agentype==1"
                  class="btn"
              ...
              <div
                ...
                v-else-if="item.f_logistics==2&&agentype==3"
              >
              ...
              <div class="checkOrder" v-else-if="item.f_logistics==4&&agentype==1">
                <span class="btn" @click="goLogisticsDetails(item.couriernumber,item.kdurl)">查看物流</span>
                <span
                
                  class="btn"
                  style="background-color:#E9423A;color: #FFFFFF"
                  @click="OpenEvaluate(item.prolist[0].proid,item.ordernum,item.id)"
                 
                >评价</span>
              </div>
              <div class="checkOrder" v-else-if="item.f_logistics==3&&agentype==3">
                <span class="btn" @click="goLogisticsDetails(item.couriernumber,item.kdurl)">查看物流</span>
                <span
                  class="btn"
                  style="background-color:#E9423A;color: #FFFFFF"
...
export default {
  ...
  methods: {
    wxpay(id, expressfee) {
      ...
      //             this.$router.push({ path: "/orderDetail", query: { id: _this.orderid }});
      //             });
      //           } else {
      //             Toast({
