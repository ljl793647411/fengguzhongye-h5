    ...
    <div class="waitItem" v-for="(item,index) in data" :key="index">
      ...
      <div class="waitItemRight">
        <div class="rightList">
          ...
          <div class="suc" v-else-if="item.verifystate==2">申请通过</div>
        </div>
        <div class="rightList">
          <div>
        ...
        <div class="rightList">
          <div>
            <span>级别:{{item.present_level_name}}</span>
          </div>
...
export default {
  ...
  methods: {
    recommendAgent(obj) {
      var _this = this;
      this.$axios
        .get("upgrade.page.list", {
          params: {
            verifystate: obj,
            present_superior: 0,
            create_userid: _this.userInfo.id
          }
        })
        .then(p => {
          if (p.data.Code == 100) {
    ...
    verifystateAgent(i, v) {
      ...
      this.$axios
        .post("upgrade.verify.agent", {
          verifystate: v,
          id: aId,
          verify_agent_id: _this.userInfo.id
        })
        .then(p => {
          if (p.data.Code == 100) {
            if (p.data.Data > 0) {
