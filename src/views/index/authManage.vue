<template>
  <div
    id="authManage"
    v-infinite-scroll="recommendAgentPage"
    ...
    <div class="waitItem" v-for="(item,index) in data" :key="index">
      ...
      <div class="waitItemRight">
        <div class="rightList">
          ...
          <div class="suc" v-if="item.verifystate==1 && item.is_super=='1'" @click="verifystateAgent(index,2)">申请通过</div>
          <div
            class="suc"
            v-else-if="item.verifystate==2"
            @click="goPage('/authorization?id='+item.id)"
          >查看授权书</div>
        </div>
        <div class="rightList">
          <div>
        ...
        <div class="rightList">
          <div>
            <span>级别:{{item.levelname}}</span>
          </div>
        </div>
        <div class="rightList">
          <div>
            <span>审核类型:{{item.type_name}}</span>
          </div>
          <div class="err" v-if="item.verifystate==1 && item.is_super=='1'" @click="verifystateAgent(index,3)">申请驳回</div>
      ...
      this.$axios
        .get("team.audit.page.list", {
          params: {
            ...
            pagesize: this.pagesize,
            present_superior:
              _this.userInfo.usertype == 2 ? 0 : _this.userInfo.id
          }
        })
        .then(p => {
