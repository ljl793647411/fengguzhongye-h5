...
微信配置
每次url变化之后都需要重新微信jssdk授权，虽然每次授权url除去#后都是一样的，但是必须这么做，微信的机制。
还有一个需要注意的坑，使用微信自定义分享功能的时候，当分享链接中存在中文的时候，一定要进行encodeURIComponent()，
否则在安卓手机上能成功自定义分享，ios手机上则不能成功分享。
查阅资料后得知是安卓手机会自动encodeURIComponent，而ios不会。
